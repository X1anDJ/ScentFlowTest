...
fragment float4 fragment_main(VOut in [[stage_in]], constant Params& P [[buffer(0)]]) {
    ...
    const float preserve = 0.75; // 0 = physical, 1 = max preservation
    float gain = pow((Y_target + 1e-5) / (Y_mix + 1e-5), preserve);
    float3 Tcomp = saturate(Tmix * gain);

    // Back to sRGB color
    float3 c = linear_to_srgb(Tcomp);
